...
function _deselectCurrentSession(sid) {
  ...
  uiModule.el('current-meta').textContent = 'JUKEYMAN Chat';
...
export async function selectSession(id, { keepSidebar = false } = {}) {
  ...
  try {
    ...
    // URL hash — the user complained that coming back to JUKEYMAN kept
    ...
    if (currentMetaEl) {
      currentMetaEl.textContent = meta ? meta.name : 'JUKEYMAN Chat';
    }
