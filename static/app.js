...
//  document.getElementById('chat-form').addEventListener('submit', chatModule.handleChatSubmit);
  ...
  // Higgs Studio tool button
  const toolHiggsBtn = el('tool-higgs-btn');
  const higgsModal = el('higgs-modal');
  const higgsIframe = el('higgs-iframe');
  const closeHiggsModal = el('close-higgs-modal');
  const higgsPopoutBtn = el('higgs-popout-btn');

  if (toolHiggsBtn && higgsModal) {
    toolHiggsBtn.addEventListener('click', () => {
      if (higgsIframe && !higgsIframe.src) {
        higgsIframe.src = '/higgs-studio/';
      }
      higgsModal.classList.remove('hidden');
    });
  }

  if (closeHiggsModal && higgsModal) {
    closeHiggsModal.addEventListener('click', () => {
      higgsModal.classList.add('hidden');
    });
  }

  if (higgsPopoutBtn) {
    higgsPopoutBtn.addEventListener('click', () => {
      window.open('/higgs-studio/', '_blank');
    });
  }
