...
def _rewrite_url_for_cf_gateway(url: str, provider: str) -> str:
    """Rewrite target API endpoints to route through Cloudflare AI Gateway if configured.
    
    This proxies, logs, caches, and automatically fails over upstream requests at the edge.
    """
    import os
    ...
    # Structure URLs: https://gateway.ai.cloudflare.com/v1/{account_id}/{gateway_id}/{provider}
    if provider == "openai":
        return f"{gateway_url}/openai/v1/chat/completions"
    elif provider == "anthropic":
        return f"{gateway_url}/anthropic/v1/messages"
    elif provider == "groq":
        return f"{gateway_url}/groq/v1/chat/completions"
    elif provider == "deepseek":
        return f"{gateway_url}/deepseek/v1/chat/completions"
    elif "nvidia" in url or provider == "nvidia":
        return f"{gateway_url}/nvidia/v1/chat/completions"
...
def llm_call(url: str, model: str, messages: List[Dict], temperature: float = LLMConfig.DEFAULT_TEMPERATURE,
    ...
    target_url = _rewrite_url_for_cf_gateway(target_url, provider)
    try:
