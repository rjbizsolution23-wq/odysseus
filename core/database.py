...
def init_db():
    ...
    _seed_nvidia_endpoint()


def _seed_nvidia_endpoint():
    """Seed the NVIDIA Developer Platform NIM endpoint if NVIDIA_API_KEY is configured."""
    nv_key = os.getenv("NVIDIA_API_KEY")
    if not nv_key:
        return
    try:
        db = SessionLocal()
        # Check if an endpoint with nvidia URL already exists
        exists = db.query(ModelEndpoint).filter(
            (ModelEndpoint.base_url == "https://integrate.api.nvidia.com/v1") |
            (ModelEndpoint.name == "NVIDIA Developer Platform")
        ).first()
        if not exists:
            import json
            import uuid
            # Seed default models in cached_models
            default_models = [
            ...
            ep = ModelEndpoint(
                id=str(uuid.uuid4()),
                name="NVIDIA Developer Platform",
                base_url="https://integrate.api.nvidia.com/v1",
                api_key=nv_key,
                is_enabled=True,
                cached_models=json.dumps(default_models),
                model_type="llm",
                supports_tools=True
            )
            db.add(ep)
            db.commit()
            logging.getLogger(__name__).info("Seeded NVIDIA Developer Platform endpoint into database.")
        db.close()
    except Exception as e:
        logging.getLogger(__name__).warning(f"Failed to seed NVIDIA endpoint: {e}")
