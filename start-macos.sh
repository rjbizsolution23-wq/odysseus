...
# Start Higgs Studio in the background on port 7861
HIGGS_PID=""
if [ -d "higgs" ]; then
  echo "▶ Starting Higgs Studio on port 7861..."
  kill $(lsof -ti:7861) 2>/dev/null || true
  PORT=7861 pnpm --dir higgs run dev >/dev/null 2>&1 &
  HIGGS_PID=$!
fi

# Setup is done — drop the setup-failure handler, and clean up the background
# opener and Higgs Studio when the server exits or the user presses Ctrl+C.
trap - ERR
trap '[ -n "$POLLER_PID" ] && kill "$POLLER_PID" 2>/dev/null; [ -n "$HIGGS_PID" ] && kill "$HIGGS_PID" 2>/dev/null; kill $(lsof -ti:7861) 2>/dev/null || true' EXIT INT TERM
